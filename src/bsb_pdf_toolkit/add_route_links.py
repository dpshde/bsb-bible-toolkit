...
def is_heading(span):
    ...
    if not text:
        return False
    # Parenthetical spans are Bible-reference/cross-reference material in the
    # source PDF. Do not turn those into route.bible links.
    if text.startswith("("):
        return False
    if "|" in text:
        return False
...
def main():
    ...
    add_route_links(input_path, output_path)
