...
def usfm_ref_to_route(target):
    ...
    if not osis:
        return None
    return build_url(osis, int(chapter), int(start), int(end or start))
...
def parse_usfm_ref_runs(text):
    ...
    for match in pattern.finditer(text):
        ...
        runs.append({"text": normalize_run_text(display), "target": usfm_ref_to_route(target) or None})
        pos = match.end()
    if pos < len(text):
...
def extract_usfm_chapters(usfm_zip_path: Path):
    ...
    with zipfile.ZipFile(usfm_zip_path) as zf:
        ...
        names.sort(key=lambda n: BOOK_ORDER.get(USFM_TO_BOOK.get(PurePosixPath(n).stem, ""), 999))
        for name in names:
            code = PurePosixPath(name).stem
            book = USFM_TO_BOOK.get(code)
            if not book:
                continue
            osis = OSIS_BOOKS.get(book)
...
class ReflowWriter:
    def __init__(self, output_path: Path):
        ...
        self.gutter = 22
        self.column_width = (self.page_width - 2 * self.margin_x - self.gutter) / 2
        self.column = 0
    ...
    def draw_book_title(self, book):
        ...
        self.canvas.setFont("Lexend-Bold", 31)
        width = pdfmetrics.stringWidth(book, "Lexend-Bold", 31)
    ...
    def draw_paragraph(self, para, osis, chapter, initial_verse=None):
        text = para["text"]
        kind = para["kind"]
        url = para["url"]
        if kind == "heading":
            self.draw_section_heading(text, para.get("crossrefs", []), url or f"https://route.bible/{osis}.{chapter}")
            return
        elif kind == "blank":
            self.ensure_space(8)
            self.y -= 8
            ...
            if kind == "poetry":
                ...
                return
            lines = self.wrap_styled_runs(
                self.body_runs(text, para.get("verses", []), font, size, osis, chapter, initial_verse),
                self.column_width,
            )
            needed = before + leading * len(lines) + after
            self.ensure_space(needed)
            for verse in para.get("verses", []):
                self.add_destination(f"file:{para['source']}#{verse}")
            self.y -= before
            ...
            for line_runs in lines:
                ...
                for run in line_runs:
                    ...
                    self.canvas.drawString(cursor_x, self.y, run["text"])
    ...
    def draw_dropcap_paragraph(self, para, body_font, body_size, leading, before, after, osis, chapter):
        ...
        lines = self.wrap_styled_runs_variable(
            self.body_runs(text, [], body_font, body_size, osis, chapter, 1),
            line_specs,
        )
        min_drop_lines = 3
        needed = before + max(drop_size, leading * max(len(lines), min_drop_lines)) + after
        self.ensure_space(needed)
        for verse in para.get("verses", []):
            self.add_destination(f"file:{para['source']}#{verse}")
        self.y -= before
    ...
    def draw_poetry_paragraph(self, para, body_font, body_size, leading, before, after, osis, chapter, initial_verse):
        ...
        width = self.column_width - base_indent
        lines = self.wrap_styled_runs(
            self.body_runs(para["text"], para.get("verses", []), body_font, body_size, osis, chapter, initial_verse),
            width,
        )
        needed = before + leading * len(lines) + after
        self.ensure_space(needed)
    ...
    def draw_section_heading(self, text, crossrefs, url):
        title_font = "Lexend-Bold"
        title_size = 9.8
        title_leading = 12.8
        crossref_font = "Lexend-Medium"
        crossref_size = 8.7
        ...
        for line_runs in heading_lines:
            ...
            for run in line_runs:
                ...
                if target and target.startswith("file:"):
                    self.canvas.linkRect("", target, (cursor_x, self.y - 2, cursor_x + width, self.y + run["size"]), relative=0)
                elif target:
                    self.canvas.linkURL(target, (cursor_x, self.y - 2, cursor_x + width, self.y + run["size"]), relative=0)
                elif link_url:
                    self.canvas.linkURL(link_url, (cursor_x, self.y - 2, cursor_x + width, self.y + run["size"]), relative=0)
                cursor_x += width
    ...
    def body_runs(self, text, verses, body_font, body_size, osis, chapter, initial_verse=None):
        ...
        for token in self.run_tokens({"text": text, "target": None}):
            ...
            for part_text, verse_num in parts:
                ...
                runs.append({
                    "text": part_text,
                    "font": "Lexend-ExtraBold" if is_verse else body_font,
                    "size": body_size,
...
def generate(input_path: Path, output_pdf: Path, font_dir: Path):
    ...
    for chapter in chapters:
        ...
        for para in chapter["paras"]:
            writer.draw_paragraph(para, chapter["osis"], chapter["chapter"], current_verse)
...
def main():
    ...
    try:
        generate(args.input_path, args.output_pdf, args.font_dir)
