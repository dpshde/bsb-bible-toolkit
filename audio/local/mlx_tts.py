...
from audio.paths import DEFAULT_JSONL_URL, LOCAL_AUDIO_DIR

DEFAULT_JSONL = DEFAULT_JSONL_URL
...
def group_segments_for_synthesis(
    segments: Sequence[Dict[str, object]], chunk_size: int
) -> List[List[Dict[str, object]]]:
    """Group consecutive verse-level segments into larger synthesis units.

    This lets the TTS model generate continuous prosody for multiple verses
    in a single forward pass, greatly reducing the number of hard silence
    stitches that cause artifacts.
    """
    if not segments:
        return []
    if chunk_size <= 1:
    ...
    for i in range(0, len(segments), chunk_size):
        groups.append([dict(s) for s in segments[i : i + chunk_size]])
    return groups
...
def append_audio_with_pause(
    ...
    """Append audio (+ optional pause) using a small crossfade to reduce click/pop artifacts.

    Decides "first write" based on file existence or None sample_rate for robustness
    on --fresh and initial runs.
    """
    import numpy as np
    import soundfile as sf
    import os

    def _fade(arr: "np.ndarray", start: bool, length: int) -> "np.ndarray":
        if length <= 0 or len(arr) == 0:
            return arr
        length = min(length, len(arr))
        fade = np.linspace(0.0, 1.0, length, dtype=arr.dtype)
        if not start:
            fade = fade[::-1]
        out = arr.copy()
        if start:
    ...
    is_first = (sample_rate is None) or (not output_wav.exists())
    if is_first:
        sample_rate = sr
        sf.write(str(output_wav), audio, sr)
        return sample_rate
    ...
    if existing is not None and len(existing) > 0:
        to_concat.append(_fade(existing, start=False, length=fade_len))

    if pause_ms > 0:
        to_concat.append(np.zeros(int(sr * pause_ms / 1000), dtype=audio.dtype))

    if audio is not None and len(audio) > 0:
        to_concat.append(_fade(audio, start=True, length=fade_len))

    combined = np.concatenate(to_concat) if to_concat else np.array([], dtype=audio.dtype)
    sf.write(str(output_wav), combined, sample_rate)
    return sample_rate
...
def render_voice_progressive(
    ...
    sample_rate: Optional[int] = ckpt_state.get("sample_rate")
    ...
    chunk_size = max(1, getattr(args, "chunk_verses", 1))
    seg_times: List[float] = []
    run_start = time.monotonic()

    # Build groups carrying (original_index, segment) pairs
    if chunk_size <= 1:
        indexed_groups: List[List[tuple]] = [
            [(i, segments[i])] for i in range(len(segments))
        ]
    else:
        indexed_groups = []
        for i in range(0, len(segments), chunk_size):
            indexed_groups.append(
                [(j, segments[j]) for j in range(i, min(i + chunk_size, len(segments)))]
            )

    for group in indexed_groups:
        # Skip the group entirely only if *all* of its segments are already done
        group_indices = [idx for idx, _ in group]
        if all(ckpt_state["segments"][idx] for idx in group_indices):
            continue

        # Synthesize one call for the whole chunk of verses
        group_texts = [str(seg["text"]) for _, seg in group]
        synth_text = " ".join(group_texts)
        ...
        try:
            ...
            # Pause only after the last verse in this synthesis group
            last_idx = group_indices[-1]
            pause_ms = (
                int(segments[last_idx].get("pause_ms", 0))
                if last_idx < len(segments) - 1
                else 0
            )

            sample_rate = append_audio_with_pause(
                output_wav, audio, sr, pause_ms, sample_rate
            )

            if args.play_as_you_go:
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp_wav:
                    import soundfile as sf  # local for the tmp write

                    sf.write(tmp_wav.name, audio, sr)
                    play_async(Path(tmp_wav.name))
        except Exception as exc:
            preview = synth_text[:60]
            print(
                f"[{voice}] SKIP group {group_indices[0]+1}-{group_indices[-1]+1}/{total} {preview}... ({exc})",
                file=sys.stderr, flush=True,
            )
            seg_elapsed = time.monotonic() - seg_start
            seg_times.append(seg_elapsed)
            for idx in group_indices:
                ckpt_state["segments"][idx] = True
        ...
        # Use a representative preview from the group (first verse text)
        first_seg = group[0][1]
        preview = str(first_seg["text"])[:60]
        group_label = (
            f"{group_indices[0]+1}"
            if len(group_indices) == 1
            else f"{group_indices[0]+1}-{group_indices[-1]+1}"
        )
        print(
            f"[{voice}] +{done_now}/{total} (chunk {group_label}) {preview}... "
...
def _render_chapter_with_model(
    ...
    for group in indexed_groups:
        ...
        try:
            ...
            preview = synth_text[:60]
            g0, g1 = group_indices[0] + 1, group_indices[-1] + 1
            label = f"{g0}" if g0 == g1 else f"{g0}-{g1}"
            print(
                f"[{voice}] SKIP {chapter_dir_name} chunk {label}/{total} {preview}... ({exc})",
                file=sys.stderr, flush=True,
            )
            seg_elapsed = time.monotonic() - seg_start
            seg_times.append(seg_elapsed)
...
def add_pause_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Number of consecutive verses to pass to the model in one call. "
             "Larger values reduce silence-edit boundaries and let prosody flow naturally across verses. "
             "Use 1 for classic per-verse behavior.",
    )
