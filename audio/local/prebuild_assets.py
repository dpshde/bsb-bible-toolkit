...
def build_segments(book: str, chapter: str, verses: Sequence[Dict[str, object]]) -> List[Dict[str, object]]:
    """Build one segment per verse (let model handle internal prosody + punctuation).

    Previously this split on punctuation which led to many silence edits.
    """
    ...
    rows.extend(
        {
            ...
            "text": normalize_text(str(verse["text"])),
        }
        for verse in verses
    )
    ...
    segments = []
    for row in rows:
        # One segment per verse / title. Pause will be injected between groups.
        segments.append(
            {
    ...
    # Apply a reasonable verse-end pause (used at chunk boundaries)
    verse_end_pause = 650
    prev_ref = None
    for i, seg in enumerate(segments):
        ref = str(seg["displayRef"])
        if prev_ref is not None and ref != prev_ref:
            segments[i - 1]["pause_ms"] = verse_end_pause
        prev_ref = ref
    if segments:
...
def group_segments(segments: Sequence[Dict[str, object]], chunk_size: int) -> List[List[Dict[str, object]]]:
    if not segments:
        return []
    if chunk_size <= 1:
    ...
    for i in range(0, len(segments), chunk_size):
        groups.append(list(segments[i : i + chunk_size]))
    return groups
...
def prebuild_voice(args: argparse.Namespace, voice: str, segments: Sequence[Dict[str, object]]) -> Path:
    ...
    chunk_size = max(1, getattr(args, "chunk_verses", 3))
    groups = group_segments(segments, chunk_size)
    ...
    with tempfile.TemporaryDirectory(prefix="bsb-mlx-tts-") as tmp:
        ...
        for gidx, group in enumerate(groups, start=1):
            # Combine verse texts for this chunk -> one TTS call for natural flow
            group_text = " ".join(str(s["text"]) for s in group)
            segment_dir = tmp_dir / f"group{gidx:02d}"
            ...
            # Insert pause after the group using the last verse's pause value
            last_pause = int(group[-1].get("pause_ms", 0))
            is_last_group = gidx == len(groups)
            if last_pause > 0 and not is_last_group:
                audio_chunks.append(
                    np.zeros(int(sample_rate * last_pause / 1000), dtype=audio.dtype)
                )
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Verses per synthesis chunk (reduces silence artifacts by letting the model speak multiple verses continuously).",
    )
